...
float MAX30105::readTemperature() {
  ...
  //? if(system_timer_current_time() - startTime >= 100) return(-999.0);
...
{
  uint32_t markTime = system_timer_current_time();
  
  while(1)
  {
	if(system_timer_current_time() - markTime > maxTimeToCheck) return(false);
...
namespace max30105{
	...
	//%
	uint32_t getGreenSG34()
	{
		return(sg34->getGreen());
	}
